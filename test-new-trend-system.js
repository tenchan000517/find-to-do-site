...
async function testNewTrendSystem() {
  ...
  try {
    // 必要なモジュールをインポート
    const { fetchTrendingTopics, getTodaysTrendsByCategory } = require('./src/lib/trends.ts');
    const { getAllTrends } = require('./src/lib/realtime-trends.ts');
    const { categorizeAndExtractKeywords } = require('./src/lib/trend-categorizer.ts');
